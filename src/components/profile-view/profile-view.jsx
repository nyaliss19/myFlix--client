...
export class ProfileView extends React.Component {
    ...
    Axios.get(`https://nyaliss-flix-27.herokuapp.com/users/${username}`, {
      ...
      .then((response) => {
        let birthday = response.data.Birthday
          ? response.data.Birthday.slice(0, 10)
          : null;
        this.setState(
          {
          ...
          () => {
            ...
            this.setState({
              favoriteMovieList: this.findFavorites(
                this.state.movies,
  ...
  updateUserData() {
    if (this.form.current.reportValidity()) {
      ...
      Axios.put(
        `https://nyaliss-flix-27.herokuapp.com/users/${this.state.username}`,
        updatedData,
        authHeader
      )
        .then((response) => {
  ...
  removeFromFavorites(movie_id) {
    ...
    Axios.delete(
      `https://nyaliss-flix-27.herokuapp.com/users/${this.state.username}/movies/${movie_id}`,
      {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      }
    )
  ...
  unregister() {
    ...
    let reallyUnregister = window.confirm(
              ...
              <Form.Group>
                <Row>
                  ...
                  <Col>
                    <Form.Control
                      placeholder={
                        this.state.birthday ? this.state.birthday : 'yyyy-mm-dd'
                      }
          ...
          {this.state.favoriteMovieList &&
            this.state.favoriteMovieList.map((m) => (
              <Col md={3} key={m._id}>
                ...
                <Row>
                  <Button onClick={() => this.removeFromFavorites(m._id)}>
                    remove favorite
                  </Button>
                </Row>
              </Col>
